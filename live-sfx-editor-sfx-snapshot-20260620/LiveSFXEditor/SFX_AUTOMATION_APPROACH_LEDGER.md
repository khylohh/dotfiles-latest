...
| A10 | Editorial SFX packet worker: caption-only packets with V3 moments, timing options, fold-safe examples, Codex/LLM editorial decisions, and direct product scoring | In progress; not scored yet | No score yet. | This is the next non-linear/editorial approach after A09 failed. It should be tested only by packet decisions on held-out projects, scored as matched / human total, generated, false additions, and net saved edits. Do not treat packet counts, worker completion, or example retrieval quality as product progress. |
