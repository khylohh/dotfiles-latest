...
const workerScript = resolve(editorRoot, 'scripts/sfx/run-editorial-sfx-worker.mjs');
...
function packetFilesFromRoot(rootPath) {
  ...
  const manifestPath = join(root, 'editorial-sfx-packet-manifest.json');
  if (!existsSync(manifestPath)) throw new Error(`Missing editorial-sfx-packet-manifest.json under ${root}`);
...
function main() {
  ...
  const outRoot = args.get('out-root') ? resolve(String(args.get('out-root'))) : join(packetsRoot, 'decisions-codex');
  ...
  for (const packetPath of packetFiles) {
    ...
    const outPath = join(outRoot, `${packetBase}.decision.json`);
    if (!overwrite && existsSync(outPath)) {
      skipped += 1;
      continue;
    }
    const commandArgs = [workerScript, '--packet', packetPath, '--out', outPath];
    ...
    if (result.status !== 0) throw new Error(`Worker failed for ${packetPath} with exit code ${result.status}`);
    completed += 1;
  }

  console.log(JSON.stringify({ packetsRoot, outRoot, requestedPackets: packetFiles.length, completed, skipped }, null, 2));
}
