...
from evidence_motif_model import (
...
DEFAULT_OUT_ROOT = EDITOR_ROOT / "validation/runs-a11"
...
def apply_nms(rows, seconds):
    selected = []
    suppressed = []
    for row in sorted(rows, key=lambda item: (-item["evidence"]["margin"], item["projectId"], item["targetSec"], item["momentId"])):
        if any(
...
def select_policy_on_training(records):
    best = None
    prepared_folds = [
        {**fold, "model": build_evidence_model(fold["train"])}
...
def main():
    ...
    run_id = args.run_id or datetime.now(timezone.utc).strftime("evidence-motifs-v1-%Y%m%dT%H%M%SZ")
    ...
    for fold in splits:
        ...
        model = build_evidence_model(train)
    ...
    report = {
        "schemaVersion": 1,
        "protocol": "evidence-motifs-v1",
        "createdAt": datetime.now(timezone.utc).isoformat(),
        "metricBoundary": "outer-fold-only; motifs built from trainProjectIds only",
        "enabledFamilies": ["pop", "ding", "bonk"],
    ...
    write_json(run_dir / "evidence-motif-score.json", report)
    write_jsonl(run_dir / "evidence-motif-emissions.jsonl", all_emissions)
    write_jsonl(run_dir / "evidence-motif-nms-suppressed.jsonl", all_suppressed)
